...
> F-43), two of them dating to tasks 09 and 16. Current state: **146/146 integration tests pass**
> (re-measured 2026-08-04 on `dev` @ `3d58abf`) against PostgreSQL 16.6 + pgvector 0.8.1. Task 16's Testcontainers waiver (**F-32**) no longer
...
| 25 | [Travel research agent](25-travel-research-agent.md) | 14, 17, 23, 24 | `done` | Merged to `dev`. Migration **V25**. Stub agent + KnowledgePort tools + completion-hook persistence. Handoff: [`docs/TRAVEL-RESEARCH-AGENT-HANDOFF.md`](../docs/TRAVEL-RESEARCH-AGENT-HANDOFF.md). Sample PARTIAL KB → typed `no_confident_result`. **The agent is the deterministic stub only** — no live narrative path exists (**F-52**); tasks 30/36 must not assume one. |
| 26 | [Research API and frontend](26-research-api-frontend.md) | 11, 23, 24, 25 | `done` | Merged to `dev`. Ranked list + select + guide APIs; research UI; handoff [`docs/RESEARCH-EXPERIENCE-HANDOFF.md`](../docs/RESEARCH-EXPERIENCE-HANDOFF.md). **2026-08-04:** the guide endpoint cited one hardcoded ref for the whole page, leaving POIs/areas/transport/apps uncited — fixed and pinned, **F-51**. |
...
| 28 | [Itinerary domain and scheduling](28-itinerary-domain-scheduling.md) | 17, 18, 26 | `done` | Merged `956e474` (branch tip `a4ba3a2`). Migration **V27** (`itinerary`, `itinerary_day`, `itinerary_item`); **next free V28**. Pure `domain/algorithm/scheduling/` + aggregate + persistence; feasibility is decided before narration, so task 30 cannot label a plan ready. **F-29 verified closed before starting** — the scheduler needs a real IANA zone and `Destination` enforces one. Evidence on the merge commit: `verify:task` 9/9, `test:integration` **146/146** with V27 applied and `ddl-auto: validate` accepting the entities, 46 new unit tests. Handoff: [`docs/ITINERARY-SCHEDULING-HANDOFF.md`](../docs/ITINERARY-SCHEDULING-HANDOFF.md). Known gaps recorded there, not hidden: a day cannot wrap midnight, area clustering is honoured rather than computed, `TRANSIT` only when supplied. **CI:** the code was green (run 64: backend, frontend, docker smoke, both prereqs), but the **`docs(status)` commit that marked this row `done` turned run 64 red on the `CODE-MAP is current` gate** — CODE-MAP indexes task status out of STATUS.md, so marking a task done makes it stale, and the ladder had been run *before* that commit rather than after. Fixed by task 29's regeneration; green from run 65. |
| 29 | [Route and mobility planning](29-route-mobility.md) | 17, 28 | `done` | Merged `426b717` (branch tip `6576917`). Migration **V28** `itinerary_leg`; **next free V29**. Pure `domain/algorithm/mobility/` — a four-rung ladder ending in `UNKNOWN`, with **no distance heuristic anywhere**, so "never fabricate precision" is structural rather than aspirational. UC-K14 suppression applied in one place (`TravelAppSelector`), matched on slug. `get_route` runs the same ladder as the itinerary so chat and timeline cannot disagree. Evidence: `verify:task` 9/9, `test:integration` **146/146** with V28 applied, 34 new unit tests. Handoff: [`docs/ROUTE-MOBILITY-HANDOFF.md`](../docs/ROUTE-MOBILITY-HANDOFF.md). Known gaps recorded there: no POI-level routing, no multi-hop composition, no alternatives per pair. **CI green — run 65, all six jobs** (backend Java 21, frontend Node 22, security, docker build + smoke, prereq on ubuntu and windows) on this exact tip. |
| 30 | [Itinerary generation agent](30-itinerary-agent.md) | 14, 26, 28, 29 | `done` | Merged `5662f30` (branch tip `8cb907a`). No migration. Pipeline: propose → ground → schedule → route → persist → `ITINERARY_READY`. **The model cannot express a schedule** — `ItineraryProposal` has nowhere to put a time — so the task 28 scheduler and task 29 route policy are authoritative by construction rather than by convention. Grounding lives in the agent adapter (ArchUnit caught the first attempt at putting it in `application/`). Bounded loop of 2; six typed failures. Wired agent is the deterministic stub, as on C2 (**F-52**). Evidence: `verify:task` 9/9, `test:integration` **146/146**, 33 new unit tests. Handoff: [`docs/ITINERARY-AGENT-HANDOFF.md`](../docs/ITINERARY-AGENT-HANDOFF.md). **Two scope items deferred to task 31 rather than half-built:** preserving user-approved constraints across regeneration, and `patch_itinerary`'s command shape — both depend on UI affordances that do not exist yet. |
...
> **One row per `F-nn`, and the number is never reused. Next free: `F-53`.**
>
> Deduplicated 2026-08-04. The register had drifted into three tables holding **five** rows whose
> IDs collided: F-39 and F-40 each appeared twice (a stale open statement above the later ✅ CLOSED
> one), and F-41, F-42 and F-43 each named **two different defects** — so "F-42 is closed" could not
> be resolved without guessing which F-42 was meant. `docs/COMPARE-AGENT-PRS-VS-DEV.md` §2 recorded
> the drift on 2026-08-03 and prescribed this cleanup. The three colliding open items kept their text
> and were renumbered **F-47**, **F-48**, **F-49**; the stale open duplicates of F-39/F-40 are
> deleted, because the CLOSED rows below supersede them. A number that means two things is worse than
> no register, because it is quoted as though it means one.
...
| **F-27** | ⚠️ **HALF CLOSED** (verified 2026-08-04). The record half is done — `DestinationArea` now rejects a latitude outside ±90 and a longitude outside ±180, matching `Destination` and `Poi`. **The schema half is not:** V14 still carries only `ck_destination_area_coordinates_paired`, with no range constraint, while the `destination` table has both `ck_destination_latitude_range` and `ck_destination_longitude_range`. Every current write path constructs the record, so the gap is unreachable today; it opens the day task **41** writes an area by SQL. Fix in the migration that next touches `destination_area` |
| **F-28** | ✅ **CLOSED** (verified 2026-08-04). `KnowledgeQuery` now overrides `equals`/`hashCode` and compares the embedding with `Arrays.equals(float[], float[])` rather than by reference, so two queries built from identical inputs are equal. Task 37's semantic cache can key on it |
| **F-29** | ✅ **CLOSED** (verified 2026-08-04, immediately before starting task 28 — which is what this blocked). `Destination` rejects a blank name, and `requireIanaZone` resolves the timezone through `ZoneId.of` **and** asserts it is region-based, so `"UTC+7"` and `"GMT+09:00"` are refused as the fixed offsets they are rather than accepted as zones that ignore DST. Rejecting at construction moves the failure to the seed loader, with the offending file still on screen, instead of to the C3 scheduler months later with the bad value already curated into the database. Pinned by `DestinationTest`: 5 rejected timezones (including blank and `"Tokyo"`), 4 accepted, and `rejectsABlankNameThatTheNotNullColumnWouldHappilyStore`. Residual, not part of this row: the `destination` table has no `name`-not-blank CHECK, unlike `poi` and `destination_guide.overview` |
...
| **F-37** | ✅ **CLOSED** (verified 2026-08-04). `DevAdminSeeder.run()` used to call its own `@Transactional seed()`, so the proxy was bypassed and the annotation never applied. The write now lives in `DevAdminSeedWriter.seed()` under `@TransactionalWrite`, and `DevAdminSeeder` holds it as a collaborator — a cross-bean call, so the proxy is in the path. Same split the research-complete mail listener uses (`ResearchCompletionMailWriter`) |
...
| **F-40** | ✅ **CLOSED** (verified 2026-08-04). The client parsed 3 of the contract's 6 chat roles, so the first `tool_call` history row would have failed zod and blanked the whole page rather than one message. `chat-api.ts` now publishes `ChatMessageRole` with all six literals in `roleSchema`, and *parsing* is deliberately separated from *rendering*: `ChatRole` in `chat-events.ts` stays the narrower renderable set, and `chat-state.ts` decides per role which reach the transcript (`isRenderable`) — so the wire vocabulary grows with the server's while the UI opts in deliberately, per §7.2's ban on raw tool JSON |
| **F-47** | ✅ **CLOSED** (verified 2026-08-04). *Was a second, colliding use of the number F-41.* `fetchChatHistory` re-sorted a page by `created_at`, the one key that provably cannot order a conversation: `now()` is fixed for a whole transaction in Postgres, so a turn's tool call, tool result and assistant reply carry identical stamps. The old comparator returned 0 for exactly those rows and leaned on `Array.prototype.sort` being stable — trusting the server's order while appearing not to. Now `compareBySeq`, with the reasoning recorded on the function |
| **F-48** | ✅ **CLOSED** (verified 2026-08-04). *Was a second, colliding use of the number F-42.* `surprise_me` (UC-C1-05) had no persistence column and no `TripBriefDetails` field, so the flag was lost on save. Migration **V23** `add_trip_brief_surprise_me` adds the column and `TripBriefDetails.surpriseMe` carries it; wired end to end through the extraction save path (gate **19B**) |
| **F-49** | ✅ **CLOSED** (verified 2026-08-04). *Was a second, colliding use of the number F-43.* `ai_call_log.total_tokens` was always 0 for `complete()` calls — a `String` has nowhere to carry usage, so every non-streaming call, including every structured-extraction attempt, wrote a zero-token zero-cost row. `LlmClientRouter.complete` now routes through `completeWithTools(prompt, List.of(), options)` and takes `.text()`: the same provider call, but the one whose result still holds the counts when the router records it. Task 14's port contract is unchanged |
...
| **F-50** | ✅ **CLOSED** 2026-08-04 (found auditing tasks 16–27 for completeness). **Two of task 17's six DoD bullets were never built.** The domain had both capabilities all along — `KnowledgeProvenance.isSampleData()` and `.isStaleAt(now, dataClass)` — but `isSampleData()` had **zero production call sites**, `isStaleAt` was called only by `FreshnessCalculator` to compute a ranking penalty, and neither `sample_data` nor `stale` appeared in the contract, in any DTO, or anywhere in the frontend. So the API served `stub:sample` PARTIAL content as though it were curated fact, and a row past its TTL as though it were current — the exact "presented as current" failure `KnowledgeDataClass`'s own javadoc says the flag exists to prevent. Compounding it, `SupportedDestinations` had been made an object *specifically* to hold this key (its schema comment says so) and the key was never added. Fixed by publishing the truth at both granularities: **`KnowledgeSourceRef`** (new contract schema) carries `sample_data`, `stale`, `attribution` and `retrieved_at` per citation, and `sample_data` is a page-level flag on `DestinationGuideDetail` plus `SupportedDestinations`. Staleness is decided from a new `dataClass()` on each knowledge record rather than a mapping table at the boundary, so a caller cannot pick the wrong TTL for a POI. Pinned by `DestinationGuideDetailResponseTest` (9 tests, including a fresh narrative beside a stale POI on one page) and two `DestinationControllerTest` cases |
| **F-51** | ✅ **CLOSED** 2026-08-04. **Task 26's guide endpoint cited one hardcoded ref.** `DestinationGuideDetailResponse.from` emitted a single `source_refs` entry — the guide's own provenance, with `field_group` hardcoded to the string `"overview"` — so the `food` and `practical` prose was attributed to a source that had not been checked for it, and the POIs, areas, transport modes and app pack rendered beside them carried **no citation at all**, despite each holding its own `KnowledgeProvenance` in the domain. An uncited fact on a page that displays citations reads as sourced, which is what PLAN §4.1.0 exists to prevent. Now one ref per `(source_ref, field_group)` pair that actually backs something, deduplicated in render order, with a section that was never curated contributing nothing rather than citing itself. Also closes the read half of ADR 010 §2: `attribution` is published, so the licence obligation is dischargeable on screen instead of being enforced in the database and dropped at the boundary |
| **F-52** | **Task 25 ships a stub agent only.** `StubTravelResearchAgent` is the sole implementation of `TravelResearchAgentPort`, and `ResearchAgentConfig` documents the intent — "a future live narrative agent would be a second `@Bean`". Task 25's DoD is genuinely met by it (structured recommendations or typed no-confident-result, source-linked claims, enforced budgets, fail-closed), and a deterministic stub is an explicit scope item for CI, so this is **not** a botched task. What is missing is the ledger saying so: task 25's scope line "LLM writes rationale/narrative but does not overwrite numeric fit score" has no live path, and tasks **30** (itinerary agent) and **36** must not assume one exists. Not accepted debt against task 25 — the narrative agent needs a key, and PLAN's no-keys-in-CI rule means it can only be built where one is available |
