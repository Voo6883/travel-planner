...
| 29 | [Route and mobility planning](29-route-mobility.md) | 17, 28 | `in_progress` | Started 2026-08-04 on `agent/task-29-route-mobility`. Deps satisfied: 17 `done_with_accepted_debt` (gates **17A/17B** `done`), 28 `done`. Owns `itinerary_leg` (**V28**). Consumes the KB surface task 16 already published — `findRouteSegments`, `findTransportModes`, `findTravelApps`, `findTravelAppReplacements`. No `Validation` section, so the DoD is the gate. |
