...
| 23 | [Research job platform](23-research-job-platform.md) | 07, 18, 22 | `in_progress` | Branch `cursor/task-23-research-job-15b0`. Implementation complete locally; awaiting parent PR. Migration **V24** `research_job` (status/progress/error/attempts/run-id + partial unique active-per-trip index). Durable slice in `application/research/`: `ResearchJobService` (persist-then-dispatch, 202), `LocalResearchJobExecutor` (in-process pool via `AFTER_COMMIT` event — **no Redis/queue**), `ResearchJobRunner` (90 s budget, interrupt-on-overrun), `ResearchStaleJobReconciler` (boot recovery), and the two task-25 seams `ResearchJobHandler`/`ResearchCompletionHook` (`@ConditionalOnMissingBean` no-ops). On fail: job `FAILED`+`error_code`, trip recovered to `BRIEF_COMPLETE` (no trip-level failed status). API `POST .../research/run` (202) + `GET .../research/jobs/{jobId}`; FE `features/research/` hook + `ResearchJobPanel` + `research` locales. Backend `./gradlew test` green (Docker-free); FE typecheck/lint/362 tests green. Handoff: [`docs/RESEARCH-JOB-HANDOFF.md`](../docs/RESEARCH-JOB-HANDOFF.md). |
