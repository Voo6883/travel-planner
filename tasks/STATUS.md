...
**Baseline commit:** `aa20043` · **Working branch:** `dev` · **Last updated:** 2026-08-03
...
| ID | Task | Depends on | Status | Notes |
|---|---|---|---|---|
| 16 | [Knowledge domain and schema](16-knowledge-domain-schema.md) | 07, 14, 15 | `done` | Commits `c68ff6f` (schema), `bbdc065` (domain). Migrations **V13–V18** applied against real Postgres 16.6 + pgvector 0.8.1: 12 tables, 6 partial HNSW indexes, and each guard proven to reject a bad write. Pure domain + `KnowledgePort` + typed `destination_not_covered` (registered end to end). 212 domain tests; coverage LINE 88.87% / BRANCH 81.73%. Persistence `8db9029`: 10 entities, 10 repositories, 10 mappers, adapter, native pgvector search. Handoff: [`docs/KNOWLEDGE-SCHEMA.md`](../docs/KNOWLEDGE-SCHEMA.md). **Testcontainers waived by the owner**; entity/schema alignment proven instead by `ddl-auto: validate` booting against Postgres 16.6 — see F-32. Task 17 added **V21** (hybrid fulltext); Task 19 remainder added **V22** (`surprise_me`). **Next free migration: `V23`.** New: **F-27**–**F-30**, **F-33**. |
| 17 | [Knowledge seed and retrieval](17-knowledge-seed-retrieval.md) | 14, 16 | `review` | Remainder commit `cadea8e` on `cursor/task-17-knowledge-seed-remainder-159a`; CI fix for V21 on `cursor/task-17-hybrid-ci-fix-15b0`. Prior `50c2d68`: seed format + SAMPLE loader + `GET /destinations/supported` + prod stub refusal. **Closed remainder:** (1) `./gradlew validateKnowledgeSeed` CI command (`SampleKnowledgeValidator` / `ValidateKnowledgeSeed`); (2) hybrid vector + `tsvector` fusion in `KnowledgeVectorSearch` via `HybridRetrievalScore` — proven on `street food` / `temples` (unit + stub corpus); migration **V21** extends POI GIN to tags via IMMUTABLE `knowledge_tags_text` (bare `array_to_string` is STABLE and fails 42P17 in expression indexes) and adds guide fulltext; (3) shared `KnowledgePortContractTest` for `StubDestinationKnowledgeAdapter` + `PgVectorKnowledgeAdapter` (mocked collaborators). Renamed `KnowledgeRepositoryAdapter` → `PgVectorKnowledgeAdapter`. Evidence (rerun): `./gradlew clean build` BUILD SUCCESSFUL with `DOCKER_HOST=tcp://127.0.0.1:1`; `validateKnowledgeSeed` OK (3 destinations); JaCoCo gate passed. **F-34** (real curation) and **F-33** (app suppression) remain open authoring/schema work. New: **F-45** (live hybrid SQL / HNSW EXPLAIN against Postgres — Testcontainers still waived). **Next free migration: `V23`.** Mark `done` after merge. |
...
| ~~**F-39**~~ | **Resolved (task 20 / PR #16).** ADR 007 amended: `id:` only on `message_start`/`message_end`; `Last-Event-ID` accepted and ignored for frame replay; reconnect regenerates via `client_message_id` idempotency. |
| ~~**F-40**~~ | **Resolved (task 20 / PR #16).** Client `ChatRole` matches the six published wire roles; tool/lifecycle history rows no longer fail zod; UI shows a non-JSON placeholder (§7.2). |
| ~~**F-41**~~ | **Resolved (task 20 / PR #16).** `fetchChatHistory` trusts contractual `seq` order; `created_at` re-sort deleted. |
| ~~**F-42**~~ | **Resolved (task 19 remainder / PR #15).** `surprise_me` (UC-C1-05) is persisted by V22 and lives on `TripBriefDetails`, `TripBrief`, OpenAPI, and the `trip_brief` row; extraction now saves the same details it returns. |
...
| **F-45** | Task 17 hybrid SQL (`KnowledgeVectorSearch` fused score) and partial HNSW index choice are unit-proven (slug guard, `HybridRetrievalScore`, stub corpus contracts) but not `EXPLAIN ANALYZE`d against live Postgres 16.6 + seeded rows in this environment — Docker/Testcontainers still unavailable (extends F-32). Re-verify planner picks `ix_poi_embedding_hnsw_*` and fused ranking once a DB-backed suite can run |
