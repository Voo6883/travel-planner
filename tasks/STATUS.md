...
| 17 | [Knowledge seed and retrieval](17-knowledge-seed-retrieval.md) | 14, 16 | `in_progress` | Commit `50c2d68`. Seed format + idempotent loader + SAMPLE dataset (3 destinations, 142 rows, 39 embedding chunks) seeded **PARTIAL**; reader refuses a file declaring `FULL`. `GET /destinations/supported` public per ADR 010 §4. Prod refuses any `Stub*` knowledge bean. **Gate 17B closed 2026-07-30.** Hybrid retrieval is `KnowledgeVectorSearch` (pgvector) + `KnowledgeFullTextSearch` (Postgres full-text over `poi`) fused by reciprocal rank in `KnowledgeHybridSearch`; `KnowledgeMatch.score` became `relevance` because it stopped being a cosine similarity and a silent change of meaning on that field is how a caller filters on a threshold that no longer exists. Migration **V22** fixes **F-46**. The other two 17B line items closed the same day — shared adapter contract tests with **F-32** (`KnowledgePersistenceIT`, 19 tests over all ten knowledge mappers) and the CI seed validator with **F-44** (`SampleSeedDomainCheck`, run on the only path that reads a seed file; `npm run seed:validate`). **Schema gap closed** 2026-07-30 — migration **V21** adds `travel_app_replacement`, so "the app you already have does not work here, install this instead" is data rather than something an LLM is asked to remember. Domain, persistence, `KnowledgePort.findTravelAppReplacements`, the nested `replaces` seed format and a SAMPLE row all land with it; the real Didi/Uber-shaped curation is gate **17C** (**F-34**). New: **F-33**. |
| 18 | [Trip and TripBrief core](18-trip-brief-core.md) | 06, 07, 11, 15, 17 | `review` | Commit `50c2d68`. Migration **V20** adds the brief columns V6 deferred. Trip + brief CRUD, derived clarification, `DRAFT`/`CLARIFICATION_NEEDED`/`BRIEF_COMPLETE` only, ADR 008 `expected_version` on every write with `409 version_conflict` + `details.current_version`. **Gate 18B closed 2026-08-04:** `features/intake` carries the trip list, the brief editor with debounced autosave, the clarification panel, and a 409 path that re-reads the server version and keeps the field the user is typing in rather than overwriting it; `locales/{en,ms}/trip_brief.json` covers all seven clarification prompts. `surprise_me` persists via **V23** — `dev` had already taken V21 and V22. New: **F-35**. |
| 19 | [LLM TripBrief extraction](19-llm-trip-brief-extraction.md) | 14, 18 | `review` | Commit `1262401`. Versioned prompt `trip-brief-extract@v1` gated on the SHA of the **rendered** block. Structural prompt-injection separation (template variables cannot accept user text; fenced USER message; assertion that no user text reaches the system block). Domain validation authoritative — refused values are dropped so `forDetails()` asks about them. Retry hard-capped at one repair, then deterministic fallback. 11 golden fixtures incl. injection, multilingual `ms`, timeout, malformed. **Gate 19B closed 2026-08-04:** `surprise_me` reaches `TripBriefDetails`, `TripBrief`, the entity, OpenAPI and the DTOs, and the extraction save path stores the same details it returns, so UC-C1-05's open-destination answer survives a reload instead of living only on the extraction result. |
...
| 21 | [Planner chat and trip creation](21-planner-chat-trip-creation.md) | 18, 19, 20 | `review` | Planner surface offers **only** `create_trip` (`PlannerTools`), and its arguments are validated against that schema (`CreateTripArgs`) before anything runs — an unknown tool name or malformed JSON is refused without execution, so no state change is ever driven by free text. `CreateTripHandoffService` is one short `@TransactionalWrite`: trip + empty brief, `conversation.linkToTrip`, planner session ended, and the tool-call / tool-result / lifecycle rows appended, so the pre-trip messages stay reachable as the trip's history. `trip_created` is emitted only after that commit, and a duplicate tool call returns the same `trip_id` instead of a second trip. The model never supplies an owner or any persistence id. Frontend navigates on the typed event alone (`PlannerHomePanel`); `/trips` is chat-first with suggested prompts, `/trips/{tripId}` continues the same conversation. Handoff: [`docs/PLANNER-CHAT-HANDOFF.md`](../docs/PLANNER-CHAT-HANDOFF.md). Evidence: backend `./gradlew build` green (`DOCKER_HOST=tcp://127.0.0.1:1`); frontend lint/format/typecheck/test:coverage/build green, 351 tests. Mark `done` once merged and the harness evidence gate is accepted. |
...
| **F-43** | ✅ **CLOSED** 2026-07-30. `FlywayMigrationIntegrationTest.createsExactlyTheTablesThisTaskOwnsAndNoneBelongingToLaterFeatures` asserted that `destination`, `poi`, `conversation` and `message` do **not** exist — a task-07-era rule that tasks 16 (V13–V18) and 20 (V19) legitimately superseded without updating the list. Failing since task 16, invisibly, for the same Docker reason. The list now holds only what genuinely has no migration: `booking`, `itinerary_day`, `itinerary_item`, `ranked_recommendation`, `research_job` |
