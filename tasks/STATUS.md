...
| 20 | [Conversation persistence and SSE](20-conversation-sse.md) | 06, 07, 11, 14, 15 | `review` | Persistence + SSE on `50c2d68`/`1262401`. Remainder on `cursor/task-20-sse-remainders-15b0`: **F-39** — ADR 007 amended (id only on message_start/end; Last-Event-ID accepted, reconnect regenerates via `client_message_id`); **F-40** — client `ChatRole` widened to all six contract roles; tool rows render a non-JSON placeholder; **F-41** — history no longer re-sorts by `created_at`. Mark `done` after merge. |
...
| ~~**F-39**~~ | **Resolved (task 20).** ADR 007 amended: `id:` only on `message_start`/`message_end`; `Last-Event-ID` accepted and ignored for frame replay; reconnect regenerates via `client_message_id` idempotency |
| ~~**F-40**~~ | **Resolved (task 20).** Client `ChatRole` matches the six published wire roles; tool/lifecycle history rows no longer fail zod; UI shows a non-JSON placeholder (§7.2) |
| ~~**F-41**~~ | **Resolved (task 20).** `fetchChatHistory` trusts contractual `seq` order; `created_at` re-sort deleted |
