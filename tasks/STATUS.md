...
| 30 | [Itinerary generation agent](30-itinerary-agent.md) | 14, 26, 28, 29 | `in_progress` | Started 2026-08-04 on `agent/task-30-itinerary-agent`. Deps satisfied: 14, 26, 28, 29 all `done`. The agent proposes; **task 28's scheduler and task 29's route policy stay authoritative** — neither can be overridden by model output. Reuses `StructuredOutputRunner`'s bounded repair loop. Expect the same shape as task 25: a deterministic stub is the wired bean and a live agent needs a key (**F-52**). No `Validation` section, so the DoD is the gate. |
