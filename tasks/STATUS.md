...
| 17 | [Knowledge seed and retrieval](17-knowledge-seed-retrieval.md) | 14, 16 | `done` | Remainder commit `3f9f93b` on `cursor/task-17-knowledge-seed-remainder-159a`. Prior `50c2d68`: seed format + SAMPLE loader + `GET /destinations/supported` + prod stub refusal. **Closed remainder:** (1) `./gradlew validateKnowledgeSeed` CI command (`SampleKnowledgeValidator` / `ValidateKnowledgeSeed`); (2) hybrid vector + `tsvector` fusion in `KnowledgeVectorSearch` via `HybridRetrievalScore` — proven on `street food` / `temples` (unit + stub corpus); migration **V21** extends POI GIN to tags and adds guide fulltext; (3) shared `KnowledgePortContractTest` for `StubDestinationKnowledgeAdapter` + `PgVectorKnowledgeAdapter` (mocked collaborators). Renamed `KnowledgeRepositoryAdapter` → `PgVectorKnowledgeAdapter`. Evidence: `./gradlew clean build` BUILD SUCCESSFUL with `DOCKER_HOST=tcp://127.0.0.1:1`; JaCoCo LINE 92.36% / BRANCH 86.76%; `validateKnowledgeSeed` OK (3 destinations). **F-34** (real curation) and **F-33** (app suppression) remain open authoring/schema work. New: **F-45** (live hybrid SQL / HNSW EXPLAIN against Postgres — Testcontainers still waived). **Next free migration: `V22`.** |
