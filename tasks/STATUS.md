...
| 28 | [Itinerary domain and scheduling](28-itinerary-domain-scheduling.md) | 17, 18, 26 | `in_progress` | Started 2026-08-04 on `agent/task-28-itinerary-scheduling`. Deps satisfied: 17 `done_with_accepted_debt` (gates **17A/17B** `done`), 18 and 26 `done`. **F-29 verified closed first** — the scheduler needs `Destination.timezone` to be a real IANA zone, and it is. Migration **V27** (`itinerary`, `itinerary_day`, `itinerary_item`); next free **V28**. Domain + pure scheduler + persistence landed, 46 unit tests; 146/146 ITs green with `ddl-auto: validate` accepting the new entities. Handoff: [`docs/ITINERARY-SCHEDULING-HANDOFF.md`](../docs/ITINERARY-SCHEDULING-HANDOFF.md). No `Validation` section, so the DoD is the gate. **Not yet `done`** — awaiting merge to `dev` + CI. |
