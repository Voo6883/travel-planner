...
| **F-32** | ⚠️ **PARTLY CLOSED** 2026-07-30, now that the container suite runs without Docker (`npm run test:integration`). **Closed:** the suite executes — 118 tests against PostgreSQL 16.6 + pgvector 0.8.1 — so entity/schema alignment no longer rests on `ddl-auto: validate` alone, and the HNSW question is answered by measurement: the planner chooses `ix_poi_embedding_hnsw_tokyo` above ~1–2k rows and correctly refuses it below, which means at ADR 010 §1's curated floor (~75 embeddings) **the six indexes are unused and cost write time for a corpus that does not exist yet**. Recorded in `docs/KNOWLEDGE-SCHEMA.md` §5. **Still open:** adapter round-trip coverage — `PersistenceMappingIntegrationTest` covers user/trip/brief and nothing under `KnowledgePort`, so the ten knowledge mappers have no integration test. Concurrency behaviour of the V13–V18 guards is likewise unexercised. Both are now cheap to add and belong with gate 17B |
