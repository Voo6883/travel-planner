...
| 25 | [Travel research agent](25-travel-research-agent.md) | 14, 17, 23, 24 | `review` | Branch `cursor/task-25-research-agent-15b0`. Migration **V25** `research_run_result` + `ranked_recommendation`. `TravelResearchJobHandler` + `StubTravelResearchAgent` + KnowledgePort tools; completion hook persists outcomes atomically with `RESEARCH_READY`. Handoff: [`docs/TRAVEL-RESEARCH-AGENT-HANDOFF.md`](../docs/TRAVEL-RESEARCH-AGENT-HANDOFF.md). Sample KB stays PARTIAL → typed `no_confident_result` (OK). |
